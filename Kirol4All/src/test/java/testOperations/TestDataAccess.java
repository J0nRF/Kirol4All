...
public class TestDataAccess {
	...
	public boolean removeDriver(String driverEmail) {
		...
		User d = db.find(User.class, driverEmail);
		if (d!=null) {
	...
	public User createDriver(String email, String name) {
		System.out.println(">> TestDataAccess: addDriver");
		User driver=null;
			db.getTransaction().begin();
			try {
			    driver=new User(name,email);
				db.persist(driver);
				db.getTransaction().commit();
			}
			catch (Exception e){
	...
	public boolean existDriver(String email) {
		 return  db.find(User.class, email)!=null;
		 

	}
		
		public User addDriverWithRide(String email, String name, String from, String to,  Date date, int nPlaces, float price) {
			System.out.println(">> TestDataAccess: addDriverWithRide");
				User driver=null;
				db.getTransaction().begin();
				try {
					 driver = db.find(User.class, email);
					if (driver==null)
						driver=new User(name,email);
